...
-- 交易分析-过滤表（来源最新区块实时查询定时任务，为了保证过滤掉的数据确实是无用的）
CREATE TABLE `near_analyze`.`transaction_analyze_filter`
(
    `id`               BIGINT       NOT NULL AUTO_INCREMENT,
    `signer_id`        VARCHAR(64) COMMENT '请求发起者',
    `receiver_id`      VARCHAR(64) COMMENT '请求处理者/合约',
    ...
    `create_time`      BIGINT NULL COMMENT '交易时间（要取交易时间，或者区块产生时间）',
    PRIMARY KEY (`id`)
) ENGINE=INNODB CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='交易分析过滤表（来源最新区块）';
